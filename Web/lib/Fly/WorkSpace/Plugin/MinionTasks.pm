package Fly::WorkSpace::Plugin::MinionTasks;
...
use DateTime;
use WebService::Sprites;

sub register ( $self, $app, $config ) {

    my $screenshot;

    my $sprites = WebService::Sprites->new( 
        base_url => 'https://api.sprites.dev',
        api_key  => $ENV{SPRITES_API_KEY},
    );

    $app->plugin( Minion => { Pg => $app->config->{database}->{minion} } );
    $app->plugin( 'Minion::Admin', route => $config->{route});
    
    $app->minion->add_task(destroy_workspace => sub ( $job, $sprite_name ) {
        $sprites->destroy_sprite( name => $sprite_name);
    });
    
    $app->minion->add_task(start_workspace => sub ( $job, $sprite_name ) {
        $sprites->sprite_exec( name => $sprite_name, cmd => [ 'code-server' ], timeout => 120 );
    });


    $app->minion->add_task(create_workspace => sub ( $job, $person_id, $name, $desc ) {
        my $sprite_name = "workspace-" . join("", map { ( ( 'a' .. 'z' ), ( 0 ..9 ))[int(rand( 36 ))] } ( 1 .. 6 ) );

        my $res = $sprites->create_sprite( name => $sprite_name, url_settings => 'public');

        my $hostname = $res->{url};

        $job->note( created_sprite => $res );

        $job->note( download => "Running curl -o install.sh -fsSL https://code-server.dev/install.sh" );
        $sprites->sprite_exec( name => $sprite_name, cmd => [ 'curl', '-o', 'install.sh', '-fsSL', 'https://code-server.dev/install.sh' ] );

        $job->note( install => "Running sh install.sh" );
        $sprites->sprite_exec( name => $sprite_name, cmd => [ 'sh', 'install.sh' ] );
...
#        $job->note( service_return => $res );
        ...
        if ( $res->decoded_content =~ m/^password: (.+?)$/ms ) {
            my $password = $1;

            $job->app->db->person($person_id)->create_related('sprites', {
                name         => $sprite_name,
                display_name => $name,
                description  => $desc,
                password     => $password,
