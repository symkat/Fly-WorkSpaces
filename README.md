...
You will need a Sprites API key as well as two databases.  You can create an [MPG cluster](https://fly.io/docs/mpg/) and then create a user account 'workspace` and two databases: `minion` and `workspace`.  Use the `connections` tab to get the `psql://` connection lines.
